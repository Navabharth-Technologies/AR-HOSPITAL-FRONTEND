...
export default function OPDLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("https://ar-hospital-backend-hqagfqdbbxguehdb.centralindia-01.azurewebsites.net/api/login", {
        ...
        body: JSON.stringify({ username, password, role: "OPD_HANDLER" })
      });
      const data = await res.json();
      
      if (data.success && data.user.role === "OPD_HANDLER") {
    ...
    <main className="flex min-h-screen items-center justify-center relative overflow-hidden bg-slate-50">
      ...
      >
        ...
        <div className="flex flex-col items-center space-y-4 relative z-10">
          ...
          <h2 className="text-3xl font-bold text-gray-900 tracking-wide">OPD Handler Access</h2>
        </div>

        <form onSubmit={handleLogin} className="flex flex-col space-y-6 relative z-10">
          <div className="space-y-2">
            <label className="text-sm text-gray-700 font-medium">Handler ID</label>
            <input 
              type="text" 
              required
              value={username}
              ...
              placeholder="Enter OPD Handler ID"
            />
          </div>
          <div className="space-y-2">
            <label className="text-sm text-gray-700 font-medium">Access Code</label>
            <div className="relative">
              <input 
                ...
                className="w-full bg-white border border-gray-300 rounded-lg px-4 py-3 pr-12 text-gray-900 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all"
                placeholder="••••••••"
              />
              <button
                type="button"
          ...
          <motion.button
            ...
            type="submit"
            disabled={loading}
            className="w-full relative group overflow-hidden bg-indigo-600 text-white rounded-xl py-4 flex items-center justify-center space-x-2 transition-all hover:bg-indigo-700 shadow-lg shadow-indigo-600/20 font-bold tracking-widest uppercase"
          >
            {loading ? (
              <motion.div 
                animate={{ rotate: 360 }}
              ...
              <>
                <span className="font-semibold tracking-wider z-10 relative">LOGIN TO OPD HANDLER</span>
        ...
        <div className="flex justify-center items-center space-x-2 text-gray-500 text-sm">
          ...
          <span>Restricted Clinical Zone</span>
        </div>
      </motion.div>
    </main>
  );
